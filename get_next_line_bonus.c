...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/01 20:01:57 by oimzilen          #+#    #+#             */
/*   Updated: 2024/12/07 08:56:23 by oimzilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*train_head[OPEN_MAX];
	char		*departed_train;
	...
	train_head[fd] = load_train_car(fd, train_head[fd]);
	if (!train_head[fd])
		return (NULL);
	departed_train = serve_train_departure(train_head[fd]);
	train_head[fd] = update_train_head(train_head[fd]);
	return (departed_train);
}
